...
{
	public static string ToCompactString(BigInteger n)
	{
		const uint a = 97;

		bool neg = n < 0;
		string str = BigInteger.Abs(n).ToString();
		if (n < 1000)
			return str;
...
public partial class Game : Node2D
{
	...
	public float BallValueMultiplier = 1;
	public BigInteger BallValue { get; private set; } = 1;
	#endregion
	...
	public uint Round
	{
		...
		{
			field = value;
			Gui.RoundLabel.Text = $"{value}";
		}
	}

	public const uint MAX_TOKENS = 1;
	public uint Tokens
	{
		get;
		private set
	...
	public uint TicketMultiplier = 1;
	private void OnTicketChange()
	{
		Gui.Shop.TicketUpdate();
	}
	#endregion
	...
	public override void _Ready()
	{
		...
		Machine.OnRestock += () =>
		{
			...
			ResetRound();
			Gui.RoundPopup.Show();
			Gui.RoundPopupLabel.Text = $"Round {Round}";
		};
		Machine.OnInRound += () =>
		{
			Gui.RoundPopup.Hide();
		...
		Machine.OnRoundEnd += () =>
		{
			...
			Tickets += scoreBar.Multiplier * TicketMultiplier;
			BallValue = Util.MultiplyBigInteger(Machine.MinCapsuleValue, BallValueMultiplier);
		};
		...
		if (@event is InputEventKey ke)
		{
			...
			switch (ke.Keycode)
			{
				...
				case Key.Space:
					...
					else if (Machine.State == Machine.StateEnum.LIMBO) Machine.Start();
					break;
			}
		}
	}
